    ...
    sorterPromise = global
      ...
      .then(({ order }) => {
        ...
        extras.forEach(extra => {
          ...
          let idx = order.indexOf(extra[0]);
          if (idx >= 0) return console.warn(`css-concentric-sort: Duplicate extra property '${extra[0]}' was not added`);

          // Insert extra[0] right before extra[1].
          idx = order.indexOf(extra[1]);
          if (idx >= 0) order.splice(idx, 0, extra[0]);
          else console.warn(`css-concentric-sort: Property '${extra[1]}' is missing from sass-lint/concentric.yml`);
        });
